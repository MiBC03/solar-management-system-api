...
function EditPeletrico(){
    const { clienteId } = useParams()
    let navigate = useNavigate();

    const attPeletrico = data => axios.put(`http://localhost:3030/clientes/edit/${clienteId}`, data)
    .then(() => {
    ...
    return (
        <div className='bg-light' style={{height:'100vh'}}>
            ...
            <div>
            <p style={{color: "white", textAlign: 'center', padding: "5px", background: "#2271b3"}}> Atualização de Projeto Eletrico </p>

            <Container fluid>
                <Row className="d-flex justify-content-center">

                <Col className='col-8 mt-5' >    
                <Form className="bg-white shadow p-5 d-flex flex-column justify-content-center" enctype="multipart/form-data" onSubmit={handleSubmit(attPeletrico)}>
